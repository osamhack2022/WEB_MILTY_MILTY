...
module.exports = () => {
  passport.use(new LocalStrategy({
    usernameField: 'us_id',             // username에 대한 받아오는 값
    passwordField: 'us_password',       // password에 대한 받아오는 값
  }, async (us_id, us_password, done) => {
    try {
      const exUser = await user.findOne({ where: { us_id } });  // username을 기반으로 DB에 값이 있는지 조회를 한 후, 해당 유저가 없는지 확인
      if (exUser) {
        const result = await bcrypt.compare(us_password, exUser.us_password); // 암호화
        if (result) {
